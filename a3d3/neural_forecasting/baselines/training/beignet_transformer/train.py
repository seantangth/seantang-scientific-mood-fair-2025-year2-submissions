"""
Training script for Beignet Public Transformer (Pre-LN, h=256) with CORAL domain adaptation.

Trains a Pre-LN Transformer forecaster on the Beignet public neural recording data,
using CORAL loss and mean alignment loss for domain adaptation, with learning rate
...
def train_transformer(X_tr, Y_tr, X_val, Y_val, X_target_n, std,
                      h, n_layers, n_heads, dropout,
                      coral_w, mean_w, seed,
                      epochs=400, patience=50, batch_size=32,
                      lr=5e-4, warmup_epochs=10,
                      device=None):
    """Train a single Transformer CORAL model with given config and seed.
    ...
    model = TransformerForecasterPreLN(n_ch, n_feat, h, n_layers, n_heads, dropout).to(device)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train Beignet Public Transformer Pre-LN h=256')
    ...
    print(f'\n=== Training {len(args.seeds)} seeds: h={H} L={N_LAYERS} heads={N_HEADS} '
          f'do={DROPOUT} CORAL={CORAL_W} Mean={MEAN_W} lr={LR} ===')

    seed_results = []
    for s in args.seeds:
        val_mse, state = train_transformer(
    ...
    # --- Save Checkpoints ---
    config = {
        'h': H, 'n_layers': N_LAYERS, 'n_heads': N_HEADS,
        'dropout': DROPOUT, 'coral_weight': CORAL_W, 'mean_weight': MEAN_W,
        'lr': LR, 'n_features': N_FEATURES, 'pre_ln': True,
        'epochs': EPOCHS, 'patience': PATIENCE, 'warmup': WARMUP_EPOCHS,
    }

    for s, val_mse, state in seed_results:
        path = os.path.join(args.output_dir, f'model_transformer_seed{s}.pth')
        torch.save({
